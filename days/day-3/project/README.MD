...
`deleteMessage`関数を実装しなさい。この関数は `Nat` 型の `messageId` を受け取り、対応するメッセージをウォールから削除し、`Ok` result に包まれた単位値を返します。もし `messageId` が無効であれば、この関数は `Err` result に包まれたエラーメッセージを返さなければなりません。
```motoko
